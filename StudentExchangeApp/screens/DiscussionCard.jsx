...
export default function DiscussionCard({
      ...
      <View key={index} style={styles.commentRow}>
        ...
        <View>
          ...
          <Text style={styles.likeText}>
            👍 {comment.votes?.up?.length || 0} | 👎 {comment.votes?.down?.length || 0} | Total: {score}
          </Text>
          <View style={{ flexDirection: 'row', marginTop: 4 }}>
            <TouchableOpacity onPress={() => onVote(item.id, index, true)}>
              <Ionicons
                name="arrow-up"
                size={16}
                color={(comment.votes?.up || []).includes(userId) ? '#A8E9DC' : '#888'}
                style={{ marginRight: 6 }}
              />
            </TouchableOpacity>
            <TouchableOpacity onPress={() => onVote(item.id, index, false)}>
              <Ionicons
                name="arrow-down"
                size={16}
                color={(comment.votes?.down || []).includes(userId) ? '#FF6E6E' : '#888'}
              />
            </TouchableOpacity>
          </View>
        </View>
  ...
  const CardContent = (
    <View style={previewMode ? styles.previewCard : styles.card}>
      ...
      <Text
        style={styles.discussionText}
        numberOfLines={previewMode ? 3 : expanded ? undefined : 3}
      >
        {item.description}
      </Text>
